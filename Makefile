...
AVRDUDE_CONF=-V -F -C /usr/local/etc/avrdude.conf 

AVRDUDE_FLASH_PGER=jtag3isp
AVRDUDE_FLASH_RATE=-b 250000 -B0.1
AVRDUDE_FLASH_PORT=usb

VERSION = 0.1

# Standard Arduino libraries it will import, e.g. LiquidCrystal:
ARDLIBS = Wire Wire/utility SPI EEPROM .
...
LOCALLIBS = . ble_lib ancs_lib ancs_lib/data_lib
...
ifeq "$(MODEL)" "leonardo"
 RESET_DEVICE = python -c "import time, serial, sys ; ser = serial.Serial(port=sys.argv[1], baudrate=1200, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS) ; ser.isOpen() ; time.sleep(1) ; ser.close(); time.sleep(2)" $(wildcard $(PORT))
else
...
CFLAGS = $(CDEBUG) $(DEFINES) $(CDEFS) $(CINCS) -O$(OPT) $(CWARN) $(CSTANDARD) $(CEXTRA)
CXXFLAGS = $(CDEFS) $(DEFINES) $(CINCS) -O$(OPT) $(CXXSTANDARD) $(CEXTRA)
...
SREC_CAT = /usr/bin/env srec_cat
TERM = /usr/local/bin/miniterm.py
REMOVE = rm -f
MV = mv -f
...
HEXSIZE = $(SIZE) --mcu atmega32u4 --target=$(FORMAT) applet/$(TARGET).hex
ELFSIZE = $(SIZE) -C --mcu atmega32u4 --target=$(FORMAT) applet/$(TARGET).elf
size_merge:
	@if [ -f applet/$(TARGET)-full.hex ]; then echo; echo $(MSG_SIZE_BEFORE); $(HEXSIZE); echo; fi

sizebefore:
	@if [ -f applet/$(TARGET).elf ]; then echo; echo $(MSG_SIZE_BEFORE); $(ELFSIZE); echo; fi
...
merge: applet/$(TARGET).hex $(ARDUINO_DIR)/hardware/arduino/bootloaders/caterina/Caterina.hex
	$(SREC_CAT) applet/$(TARGET).hex -Intel $(ARDUINO_DIR)/hardware/arduino/bootloaders/caterina/Caterina.hex -Intel -Output applet/$(TARGET)-full.hex

flash: sizebefore merge size_merge
	$(AVRDUDE) $(AVRDUDE_CONF) -p $(MCU) -P $(AVRDUDE_FLASH_PORT) -c $(AVRDUDE_FLASH_PGER) $(AVRDUDE_FLASH_RATE) -e -U flash:w:applet/$(TARGET)-full.hex

flash2: sizebefore
	$(AVRDUDE) $(AVRDUDE_CONF) -p $(MCU) -P $(AVRDUDE_FLASH_PORT) -c $(AVRDUDE_FLASH_PGER) $(AVRDUDE_FLASH_RATE) -e -U flash:w:applet/$(TARGET).hex

# Program the device.  
upload: applet/$(TARGET).hex
	echo 'r' > $(PORT)
	sleep 1
	$(AVRDUDE) $(AVRDUDE_CONF) -p $(MCU) -P $(PORT) -c $(AVRDUDE_PROGRAMMER) -b $(UPLOAD_RATE) -U flash:w:applet/$(TARGET).hex

monitor:
	/bin/zsh -c 'while [ ! -c ${PORT} ]; do; done'
	sleep 4
	$(TERM) $(PORT) 115200
